...
// ─── Mini Bar Chart ───────────────────────────────────────────────────────────
function MiniChart({ data, color, label, total30dLabel }: { data: { date: string; count?: number; amount?: number }[]; color: string; label: string; total30dLabel: string }) {
  ...
  return (
    <div className="glass rounded-2xl p-4 border border-slate-700/10">
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-700/10">
        <span className="text-xs text-slate-500">{total30dLabel}</span>
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard label={t.analytics.posts} value={myData.overview.posts}
              sub={`${myData.overview.posts7d} ${t.analytics.thisWeek}`}
              icon={<BarChart2 size={14} />} color="#6366f1" />
            <StatCard label={t.analytics.engagement} value={myData.overview.totalLikes + myData.overview.totalComments}
              sub={`${myData.overview.totalLikes} ${t.analytics.likes} · ${myData.overview.totalComments} ${t.analytics.comments}`}
              icon={<Heart size={14} />} color="#ef4444" />
          </div>

          {/* Social stats */}
          <div className="grid grid-cols-3 gap-3">
            <StatCard label={t.analytics.followers} value={myData.overview.followers}
              icon={<Users size={14} />} color="#a855f7" />
            <StatCard label={t.analytics.following} value={myData.overview.following}
              icon={<Users size={14} />} color="#6366f1" />
            <StatCard label={t.analytics.referrals} value={myData.overview.referrals}
              icon={<Award size={14} />} color="#f59e0b" />
          </div>

          {/* Earnings chart */}
          {myData.earningsChart && (
            <MiniChart data={myData.earningsChart} color="#10b981" label={t.analytics.earningsChart} total30dLabel={t.analytics.total30d} />
          )}

          {/* Reward breakdown */}
          {myData.rewardBreakdown && Object.keys(myData.rewardBreakdown).length > 0 && (
            <div className="glass rounded-2xl p-4 border border-slate-700/10">
              <p className="text-xs font-semibold text-slate-400 mb-3">{t.analytics.rewardBreakdown}</p>
          ...
          {myData.topPosts?.length > 0 && (
            <div className="glass rounded-2xl p-4 border border-slate-700/10">
              <p className="text-xs font-semibold text-slate-400 mb-3">{t.analytics.topPosts}</p>
              <div className="space-y-2">
          ...
          <div className="glass rounded-2xl p-4 border border-slate-700/10 flex items-center gap-3">
            ...
            <p className="text-xs text-slate-400">
              {t.analytics.memberSince} <strong className="text-white">{new Date(myData.overview.memberSince).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}</strong>
            </p>
          </div>
        </div>
      )}
      ...
      {tab === "platform" && platformData && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard label={t.analytics.totalUsers} value={platformData.overview.totalUsers}
              sub={`+${platformData.overview.newUsers7d} ${t.analytics.thisWeek}`}
            ...
            <StatCard label={t.analytics.interactions} value={platformData.overview.totalInteractions.toLocaleString()}
              sub={`+${platformData.overview.interactions7d} ${t.analytics.thisWeek}`}
              icon={<TrendingUp size={14} />} color="#f59e0b" />
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard label={t.analytics.activeCampaigns} value={platformData.overview.activeCampaigns}
          ...
          {platformData.charts && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <MiniChart data={platformData.charts.signups} color="#22d3ee" label={t.analytics.dailySignups} total30dLabel={t.analytics.total30d} />
              <MiniChart data={platformData.charts.posts} color="#6366f1" label={t.analytics.dailyPosts} total30dLabel={t.analytics.total30d} />
              <MiniChart data={platformData.charts.rewards} color="#10b981" label={t.analytics.dailyRewards} total30dLabel={t.analytics.total30d} />
            </div>
          )}
        </div>
      )}
