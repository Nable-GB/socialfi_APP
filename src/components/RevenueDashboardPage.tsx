  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={DollarSign} label={t.revenue.artistEarnings ?? t.revenue.totalRevenue} value={`${s.totalRevenue} SMFI`} color="#22d3ee" />
        ...
        <StatCard icon={Music} label={t.revenue.streamingRevenue} value={`${s.streamingRevenue} SMFI`} color="#10b981" sub={t.revenue.perPlay} />
        <StatCard icon={Gem} label={t.revenue.nftSales} value={`${s.nftSalesRevenue} SMFI`} color="#f59e0b" />
        <StatCard icon={Zap} label={t.revenue.boostRevenue} value={`${s.boostRevenue} ${t.revenue.rewardUnit ?? "SMFI rewards"}`} color="#06b6d4" />
        <StatCard icon={Share2} label={t.revenue.repostRewards} value={`${s.repostRewards} ${t.revenue.rewardUnit ?? "SMFI rewards"}`} color="#8b5cf6" />
        <StatCard icon={Users} label={t.revenue.royaltyPaid} value={`${s.totalRoyaltyPaid} ${t.revenue.payoutUnit ?? "SMFI paid out"}`} color="#ec4899" sub={t.revenue.toNFTHolders} />
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard icon={DollarSign} label={t.revenue.fanRewards ?? t.revenue.royaltiesReceived} value={`${s.totalRoyaltiesReceived} SMFI`} color="#22d3ee" />
        <StatCard icon={Share2} label={t.revenue.repostRewards} value={`${s.repostRewards} SMFI`} color="#10b981" />
        <StatCard icon={Zap} label={t.revenue.boostSpent} value={`${s.boostSpent} SMFI`} color="#f59e0b" />
            ...
            {data.holdings.map((h: any) => (
              <div key={h.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(30,41,59,0.3)", border: "1px solid rgba(100,116,139,0.08)" }}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] text-cyan-400">{t.revenue.royalties} {Number(h.totalRoyaltyReceived).toFixed(2)} SMFI</p>
                </div>
          ...
          <div className="space-y-1.5">
            {data.recentPayouts.map((p: any) => (
              <div key={p.id} className="flex items-center justify-between p-2 rounded-lg" style={{ background: "rgba(30,41,59,0.2)" }}>
                ...
                <span className="text-xs font-bold text-cyan-400">+{Number(p.amount).toFixed(2)} SMFI</span>
              </div>
            ))}
          </div>
        </div>
